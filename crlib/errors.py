class ReactionError(Exception):
    ...
    def get(self, as_string: bool = True):
        return (
            ", ".join(i.strip() if isinstance(i, str) else i for i in self.argc)
            if as_string
            else self.argc
        )
...
class ValenceError(Exception):
    ...
    def get(self):
        return self.element, self.valence
...
class ElementsError(Exception):
    ...
    def get(self, as_string: bool = True):
        return (
            ", ".join(i.strip() for i in self.elements) if as_string else self.elements
        )