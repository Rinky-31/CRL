...
def cat(NCOE: bool = False):
    ...
    def wrapper(func: FunctionType):
        ...
        def argtype_checker(*a, **kw):
            ...
            res = False
            for arg, argname in zip(a, func.__code__.co_varnames[:func.__code__.co_argcount]):
...
def arm_check(metal: str, get_index: bool = False, get_element: bool = False):
    try:
        if not is_element(metal): raise ElementsError
        if get_element:
            if not metal in ARM: raise ElementsError
            return ARM[metal]
        if get_index:
            if not metal in ARM: return -1
